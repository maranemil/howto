...
while (!($success = $n->train(100, 0.01)) && $max-- > 0) {
...
$input = array(trim($r), trim($g), trim($b));  //note that you will have to write a normalize function, depending on your needs

$result = $n->calculate($input);
If ($result > 0.5) {
	echo " the dominant color is blue";
} else {
	echo " the dominant color is red";
}
*/
...
http://www.developer.com/lang/php/training-a-neural-network-in-php.html
https://michael-feichtinger.at/blog/2012/11/neurale-netzwerke-in-php-programmieren/
http://ileriseviyephp.blogspot.de/2015/12/php-ile-yapay-sinir-ag-denetimli.html
http://www.informit.com/articles/article.aspx?p=30596&seqNum=6


https://www.drupal.org/files/issues/class_neuralnetwork.php_.txt
...
https://github.com/infostreams/neural-network/blob/master/example.php

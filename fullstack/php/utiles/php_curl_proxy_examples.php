...
function getProxy()
{
    $data = json_decode(file_get_contents('https://gimmeproxy.com/api/get/8bb99df808d75d71ee1bdd9e5d/?timeout=0'), 1, 512, JSON_THROW_ON_ERROR);
...
for ($i = 0; $i < 5; $i++) {
    try {
        echo trim(get('https://icanhazip.com/')) . "\n";
...
const CURL_LOG_FILE = __DIR__ . '/request.txt';
...
function getDom(string $url, bool $post = false)
{

    $f = fopen(CURL_LOG_FILE, 'ab+'); // curl session log file

...
ffmpeg -i output2.mp4 -vf "crop=iw/2:ih:0:0,split[left][tmp];[tmp]hflip[right];[left][right] hstack" -threads 2 -y outputH.mp4

# Mirror Effect Vertical
ffmpeg -i output2.mp4 -vf "split [main][tmp]; [tmp] crop=iw:ih/2:0:0, vflip [flip]; [main][flip] overlay=0:H/2" -y -threads 2 outputV.mp4
